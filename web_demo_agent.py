...
from agents.intent import IntentReg
...
def combine_history(prompt):
    ...
    meta_instruction = ("你现在是一名道路安全规则专家，你需要帮助用户解答各种交通规则问题以及向用户提供驾驶车辆需要了解的各种知识，你需要给出专业、可靠、有逻辑的回答，同时用词还需要具有亲和力。")
...
async def main():
    ...
    if prompt := st.chat_input("What is up?"):
        ...
        result=None
        role_intent = IntentReg()
        # result = await role_intent.run(str([msg for msg in st.session_state.messages if msg["role"] == "user"][:5]))
        result = await role_intent.run(str(next((msg for msg in reversed(st.session_state.messages) if msg["role"] == "user"))))

        print("*****************************************************")
        print(result)
        print(type(result))
        print("*****************************************************")
       # result = await role_intent.run("".join([msg["content"] for msg in st.session_state.messages[-min(5, len(st.session_state.messages)):] if msg["role"] == "user"]))
        # result = await role_intent.run("".join([msg for msg in st.session_state.messages[-5:] if msg["role"] == "user"]))
