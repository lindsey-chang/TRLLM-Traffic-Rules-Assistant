...
启用RAG后，观察到与 TRLLM-v2-4bit 的输出稍有不同。这种现象可能是由于量化后的模型在应对过拟合情况时略有改善，从而改善了知识覆盖的情况。然而，基于量化后的TRLLM-v2-4bit表现显著下降，而RAG的性能则非常依赖于基座模型，导致RAG输出的结果并不理想。
...
- 尝试权重调整：在生成过程中调整分配给LLM和RAG组件的相对权重，以找到适用于模型的特定用例的最佳平衡。
